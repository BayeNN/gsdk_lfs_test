...
static fifo_size_t length(struct fifo *fifo, uint8_t *head, uint8_t *tail)
{
...
{
  return fifo->buffer_size - fifo_length(fifo);
}

fifo_size_t fifo_read(struct fifo *fifo, uint8_t *buffer, fifo_size_t count)
{
  count = MIN(count, fifo_length(fifo));
  ...
  if (count > 0) {
    if (fifo->flags & fifo_flag_full) {
      fifo->flags &= ~fifo_flag_full;
    }
    if (fifo->flags & fifo_flag_dma_full) {
      fifo->flags &= ~fifo_flag_dma_full;
    }
  }
...
{
  count = MIN(count, fifo_space(fifo));
  fifo_size_t bytes_left = count;
...
void dump_fifo(struct fifo *fifo)
{
  printf("%d %d ", fifo_length(fifo), fifo_space(fifo));
  dump(fifo->buffer, 5);
}
...
int main()
{
  ...
  assert(fifo_space(&fifo) == 5);

  bytes = fifo_write(&fifo, write_ptr, 255);
  assert(bytes == 5);
  assert(fifo_length(&fifo) == 5);
  assert(fifo_space(&fifo) == 0);
  write_ptr += bytes;

  bytes = fifo_read(&fifo, read_ptr, 255);
  assert(bytes == 5);
  assert(fifo_length(&fifo) == 0);
  assert(fifo_space(&fifo) == 5);
  read_ptr += bytes;

  write_ptr += fifo_write(&fifo, write_ptr, 3);
  read_ptr += fifo_read(&fifo, read_ptr, 255);

  write_ptr += fifo_write(&fifo, write_ptr, 255);
  read_ptr += fifo_read(&fifo, read_ptr, 255);
  ...
  fifo_init(&fifo, 0);
  assert(fifo_space(&fifo) == 5);
  assert(fifo_dma_space(&fifo) == 5);

  bytes = fifo_dma_reserve(&fifo, &write_ptr, 5);
  assert(bytes == 5);
  assert(fifo_dma_space(&fifo) == 0);

  fifo_dma_set_tail(&fifo, write_ptr + 5);
  bytes = fifo_read(&fifo, NULL, 5);
  assert(bytes == 5);
  assert(fifo_dma_space(&fifo) == 5);
}
#endif
