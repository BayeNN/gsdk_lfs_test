  ...
  switch (app_state) {
    ...
    case SELECT_KEY_USAGE:
      ...
      if (enter_press) {
        ...
        if ((symmetric_key_usage[symmetric_key_usage_select] & PSA_KEY_USAGE_COPY)
          ...
          printf("\n  . Wrapped key should be NON-COPIABLE!\n");
          print_key_storage();
        } else {
    ...
    case COPY_KEY:
      ...
      if ((symmetric_key_storage_select ^ 0x01) == VOLATILE_PLAIN_KEY) {
        copy_key_id = copy_key(0, DEFAULT_KEY_USAGE, SYM_KEY_ALGO);
      } else {
        copy_key_id = copy_key(PERSISTENT_COPY_KEY_ID, DEFAULT_KEY_USAGE,
                               SYM_KEY_ALGO);
      }
      if (copy_key_id == PSA_SUCCESS) {
    ...
    case DESTROY_KEY:
      ...
      if (destroy_key() == PSA_SUCCESS) {
        if (key_process == 0) {
          ...
          // Import key is a non-exportable and non-copiable key
          print_key_storage();
        } else {
          // Destroy copied key
